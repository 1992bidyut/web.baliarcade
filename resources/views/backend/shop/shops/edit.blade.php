	  ...
	  <div class="container-fluid">
	    <!-- Small boxes (Stat box) -->
			<div class="card">
			  ...
			  <!-- /.card-header -->
			  			<div class="card-body">
			  				<form action="{{ route('update.shop',$data->id) }}" method="post" enctype="multipart/form-data">
			  					@csrf
			  					<div class="row">
			               	 			 ...
			               	 			 <div class="form-group">
			  								...
			  								<select name="shopcategory_id" id="shopcategory_id" class="form-control @error('shopcategory_id') is-invalid @enderror">
			  									...
			  									@foreach($shopcategory as $row)
			  										<option @if($row->id == $data->shopcategory_id) selected="" @endif value="{{ $row->id }}">{{ $row->name_en }} | {{ $row->name_bn }}</option>
			  									@endforeach
			  								</select>
			  							</div>
